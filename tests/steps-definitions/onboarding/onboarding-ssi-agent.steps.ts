...
When(/^user tap Validate button on SSI Agent Details screen$/, async function () {
  ...
  let bootUrlError = false;
  let connectUrlError = false;
  let currentUrl = "";
  await browser.waitUntil(
    async () => {
      currentUrl = await browser.getUrl();
      bootUrlError = await SsiAgentDetailsScreen.bootUrlError.isDisplayed().catch(() => false);
      connectUrlError = await SsiAgentDetailsScreen.connectUrlError.isDisplayed().catch(() => false);
      return bootUrlError || connectUrlError || currentUrl.includes("profile-setup");
    },
    {
      timeout: 15000,
      timeoutMsg: "Neither error appeared nor navigation happened after tapping Validate",
    }
  );
